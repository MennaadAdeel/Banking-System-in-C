...
EN_cardError_t getCardExpiryDate(ST_cardData_t* cardData){
    ...
    // check the string requirements in length and non-Null pointer
    if(checkStringValidation(cardData->cardExpirationDate, HOLDER_MAX_CHAR, HOLDER_MIN_CHAR) == VALID){
...
EN_cardError_t getCardPAN(ST_cardData_t* cardData){    
    ...
    // check the string requirements in length and non-Null pointer
    if(checkStringValidation(cardData->primaryAccountNumber, HOLDER_MAX_CHAR, HOLDER_MIN_CHAR) == VALID)
        return OK; //  string entered by user meets the PAN requirements
